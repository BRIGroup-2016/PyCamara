...
class ModeloPolitico:
    ...
    def __gera_modelo(self, raiz, n_minimo_discursos, agrupamento='politico'):
        ...
        treino, teste, treino_target, teste_target = sklearn.model_selection.train_test_split(dataset, target,
                                                                                             test_size=0.1)

        normalizador = sklearn.preprocessing.MaxAbsScaler()
        regressao = sklearn.linear_model. \
            LogisticRegressionCV(multi_class='multinomial', Cs=[10, 1, 0.1, 0.01], cv=3,
        ...
        print(sklearn.metrics.classification_report(teste_target, predicao_teste, target_names=significado_labels))
